...
class Employer {
  ...
  meals() {
    return this.employees().reduce(function(acc, employee, i, arr) {
      for(let i = 0; i<employee.meals().length; i++) {
        if(!(acc.includes(employee.meals()[i]))) {
          acc.push(employee.meals()[i]);
        }
      }
      return acc
    }.bind(this), [])
  ...
  mealTotals() {
    ...
    const allTheMeals = this.employees().reduce(function(acc, em) {
      acc = acc.concat(em.meals())
      return acc
    }, [])
    allTheMeals.forEach(function(meal) {
      for(const key in mealTotal) {
        if (meal.id == key) {
          mealTotal[key]++
        }
      }
